...
var BleAdapter = require ('./ble-adapter-dfu').BleAdapter;
var TdTR4 = require ('./ble-adapter-dfu').TdTR4;
var TdDFU = require ('./ble-adapter-dfu').TdDFU;
...
var Plug_Rtc = require('../rb2/plug_rtc');
...
G_OnDFU_upPacket = function( status, id_pkt ) {    
    console.log('G_OnDFU_upPacket (id = ' + id_pkt.id + ') pkt.len = ' + id_pkt.pkt.length);
    //plug_it.on_UPPKT_callback( id_pkt );
}
G_OnDFU_Connected = function( id ) {
    var id_status = { id : id, status : true };
    console.log("OnDFU_Connected: id_status =", id_status);
    //plug_it.up_connectionStatus(id_status);
}
G_OnDFU_Disconnected = function( id ) {
    var id_status = { id : id, status : false };
    console.log("OnDFU_Disconnected: id_status =", id_status);
    //plug_it.up_connectionStatus(id_status);
    //devArray[this.id] = null; //{}
}
    ...
    BleAdapter.ScanForDevice(id, (error, device) => {
        ...
        _tr4.connect( (error) => {
            if(error) {
                ...
                _dfu.afterDeviceConnected( (error) => {
                    _dfu.on('dfu:disconnected', G_OnDFU_Disconnected )
                    _dfu.on('dfu:upPacket', G_OnDFU_upPacket )
                });
            }
