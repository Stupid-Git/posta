...
class nobleDfu extends EventEmitter {
    ...
    _transferStartPacket(file) { //karel
        return file.loadData().then(data => {
            return this._transport.sendStartPacket(data)
                .then(() => { /*console.log('$%#&%#&%#%#%#');*/ this.emit('transferComplete', file.name) });
        });
    }
    ...
    _transferValidate() { // karel
        return this._transport.send_validate_firmware()
            .then(() => { /*console.log('Done send_validate_firmware');*/ this.emit('transferComplete', 'Bogus Validate') });
    }

    _transferActivate() { // karel 
        return this._transport.send_activate_firmware()
            .then(() => { /*console.log('Done send_activate_firmware');*/ this.emit('transferComplete', 'Bogus Activate') });
    }
